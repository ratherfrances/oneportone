...
(function($, window, document, undefined) {
    'use strict';
    var V = function(bn, options) {
        this.v = bn, this.options = $.extend({}, this.defaults, options)
    };
    V.prototype = {
        defaults: {
            target: 'img.slip',
            ...
            aX.each(function() {
                ...
                if (C.options.autoScroll && F - ax > 0) {
                    ...
                    var scrollTop = {
                        ...
                        J = (F - ax) * C.options.scrollSpeed;
                    j.hover(function() {
                        C.aQ(G, scrollTop, ad, J);
                    }, function() {
                        j.children().stop().css('top', '0');
                    })
                };
            });
            return v;
        },
        aQ: function(v, az, av, J) {
            var C = this;
            v.animate(az, J, 'linear', function() {
                v.animate(av, J, 'linear', function() {
                    C.aQ(v, az, av, J);
                });
            });
        },
        al: function(R) {
            var aR = R.parents().filter(function() {
                    return this.style.position !== 'static';
                }).length,
                aL = aR ? R.position() : R.offset(),
                as = R.outerWidth(),
                bg = R.outerHeight(),
                borderWidth = (R.outerWidth() - R.innerWidth()) / 2,
                aP = aL.top,
                bI = aL.left,
                T = $('<div class="sliphoveritem" style="border-radius:' + R.css('borderRadius') + ';width:' + as + 'px;height:' + bg + 'px;text-align:center;overflow:hidden;position:absolute;top:' + aP + 'px;left:' + bI + 'px;">').insertBefore(R);
            if (this.options.flip) {
                T.css({
                    'perspective': '500px',
            ...
            var browser = this.bx();
            if (browser.k && browser.version <= 10) {
                T.css('background-color', 'rgba(255,255,255,0.1)');
            };
            return T;
        },
        bc: function(T, options, R) {
            var j = $('<div class="sliphoveritemTitle" style="border-radius:' + R.css('borderRadius') + ';width:100%;height:' + this.options.height + ';box-sizing:border-box;-moz-box-sizing:border-box;padding:30px;overflow:auto;position:absolute;color:' + this.options.fontColor + ';background-color:' + this.options.backgroundColor + ';">').html('<div style="position:relative;width:100%;">' + R.attr(options.title) + '</div>'),
                G = j.children();
            if (this.options.flip) {
                j.css({
                    'left': '0',
        ...
        H: function(bV, bM, T, j) {
            ...
            T.css(bV);
            j.css({
                'animation': bM + ' ' + duration + ' forwards',
                '-webkit-animation': bM + ' ' + duration + ' forwards'
            });
        },
        aO: function(R, event) {
            var w = R.width(),
                K = R.height(),
                x = (event.pageX - R.offset().left - (w / 2)) * (w > K ? (K / w) : 1),
                y = (event.pageY - R.offset().top - (K / 2)) * (K > w ? (w / K) : 1),
                direction = Math.round((((Math.atan2(y, x) * (180 / Math.PI)) + 180) / 90) + 3) % 4;
            return direction;
        },
        aZ: function(R, j, options) {
            var C = this;
            R.unbind('mouseenter.sliphover mouseleave.sliphover').bind('mouseenter.sliphover mouseleave.sliphover', function(e) {
                var t = e.type,
                    direction = C.aO(R, e);
                C.bi(t, direction, R, j, options, C.aV);
            });
        },
        bi: function(t, direction, T, j, options, L) {
            if (options.reverse) {
            ...
            switch (direction) {
                case 0:
                    if (t == 'mouseenter') {
                        if (options.flip) {
                            this.H(this.O.bK, 'topenter', T, j);
                        } else {
                            j.css(L.bE);
                            j.stop().animate(L.l, options.duration);
                        };
                    } else {
                        if (options.flip) {
                            this.H(this.O.bK, 'topleave', T, j);
                        } else {
                            j.stop().animate(L.bE, options.duration);
                        };
                    }
                    break;
                case 1:
                    if (t == 'mouseenter') {
                        if (options.flip) {
                            this.H(this.O.bG, 'rightenter', T, j);
                        } else {
                            j.css(L.bD);
                            j.stop().animate(L.l, options.duration);
                        };
                        ...
                        if (options.flip) {
                            this.H(this.O.bG, 'rightleave', T, j);
                        } else {
                            j.stop().animate(L.bD, options.duration);
                        };
                    }
                    break;
                case 2:
                    if (t == 'mouseenter') {
                        if (options.flip) {
                            this.H(this.O.ae, 'bottomenter', T, j);
                        } else {
                            j.css(L.aD);
                            j.stop().animate(L.l, options.duration);
                        };
                        ...
                        if (options.flip) {
                            this.H(this.O.ae, 'bottomleave', T, j);
                        } else {
                            j.stop().animate(L.aD, options.duration);
                        };
                    }
                    break;
                case 3:
                    if (t == 'mouseenter') {
                        if (options.flip) {
                            this.H(this.O.an, 'leftenter', T, j);
                        } else {
                            j.css(L.aW);
                            j.stop().animate(L.l, options.duration);
                        };
                        ...
                        if (options.flip) {
                            this.H(this.O.an, 'leftleave', T, j);
                        } else {
                            j.stop().animate(L.aW, options.duration);
                        };
                    }
                    break;
                default:
    ...
    $.fn.sliphover = function(options) {
        ...
        if (!entry) {
            entry = new V(this, options);
            entry.bN();
            this.data('sliphover', entry);
        } else {
            entry.bN(this, options);
        }
